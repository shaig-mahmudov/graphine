...
Install-time inspection is separate from analysis. The supervisor invokes `--metadata`, which returns the worker version, analyzer protocol version, and packaged capability names without reading a repository or starting Maven. `graphine doctor` combines that metadata with Java/Maven executable discovery; a missing worker, malformed response, or protocol mismatch is reported as degraded rather than being inferred from local constants.
