...
for fixture in spring-web spring-beans spring-data; do
  python3 -c 'import json,sys; print(json.dumps({"protocol_version":1,"request_id":"accuracy-"+sys.argv[2],"operation":"analyze_project","project_root":sys.argv[1],"mode":"safe","source_sets":["main"],"options":{"include_method_bodies":True,"include_field_access":True,"include_tests":False,"explicit_classpath":[]},"maven_executable":"mvn","timeout_ms":120000}))' "$repo/fixtures/$fixture" "$fixture" | java -Xmx1024m -jar analyzer-jdt/analyzer-cli/target/graphine-analyzer.jar > "$temp/$fixture.jsonl"
