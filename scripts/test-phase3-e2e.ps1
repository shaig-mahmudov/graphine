...
try {
    ...
    if (@($response.result.structuredContent.facts | Where-Object kind -eq "HANDLED_BY" | ForEach-Object target) -notcontains $handler) { throw "Spring route handler edge was absent after SQLite ingestion" }
