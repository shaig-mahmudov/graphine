...
python3 -c 'import json,sys; d=json.load(sys.stdin); expected="method:dev.graphine.fixture.web.EventController#create(dev.graphine.fixture.web.CreateEventRequest)"; assert expected in [f.get("stable_id") for f in d["result"]["structuredContent"]["result"]["routes"] if f.get("relationship")=="HANDLED_BY"]; assert "RUNTIME_CONFIRMED" not in json.dumps(d)' <<<"$response"
